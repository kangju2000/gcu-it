...
def return_dataframe(ticker, start_day):
    ...
    start_30days_before = str(datetime(*map(int, start_day.split('-'))) - timedelta(days=33)).split()[
        0]  # 시작날을 입력 받으면 그 이후 33일 정도 기록누락 발생
...
def if_used(data, person, buy_value, sell_value):
    ...
    buy_data = data[
        buy_condition].copy()  # 매수할 날에 대한 데이터 프레임, 수정시 기존에 파일을 수정할지 아니면 복사본만 수정할지 정해줘야 해서 copy() 시용하여 복사본을 만들어 수정한다고 명시
